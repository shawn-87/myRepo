...
{
    public partial class EditFillUpForm : Form
    {
        EditFillUp myFillUpInfo;
        bool dateTrue = false, fuelPriceTrue = false, fillCostTrue = false, fillQuanitityTrue = false, distanceTrue = false;
        

        public EditFillUpForm(int idNumberToBe, DateTime dateTimeToBe, float fuelPriceToBe, float fuelQuantityToBe, float fuelCostToBe, float distanceToBe )
        {
            InitializeComponent();
            dateTimePickerFill.MaxDate = DateTime.Today;
            try
            {
                myFillUpInfo = new EditFillUp(idNumberToBe);
                myFillUpInfo.SetDate(dateTimeToBe);
                dateTimePickerFill.Value = dateTimeToBe;
                myFillUpInfo.SetFuelPrice(fuelPriceToBe);
                textBoxFuelPrice.Text = fuelPriceToBe.ToString();
                myFillUpInfo.SetFuelCost(fuelCostToBe);
                textBoxFillCost.Text = fuelCostToBe.ToString();
                myFillUpInfo.SetFuelQuantity(fuelQuantityToBe);
                textBoxFillQuantity.Text = fuelQuantityToBe.ToString();
                myFillUpInfo.SetDistance(distanceToBe);
                textBoxFillDistance.Text = distanceToBe.ToString();
            }
            catch
            {
                this.Close();
